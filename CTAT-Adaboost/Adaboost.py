...
class ADABoost:
    ...
    def abaBoost(self, X, y, X_train, X_test, y_train, y_test):
        '''
        Function to create the Adaboost model. The Decision tree created by DecisionTreeRegressor is boosted by the Adaboost algorithm. 
        ...
        # ada_model = AdaBoostRegressor(base_estimator = decision_tree, 
        #                         n_estimators=20000, # 20000
        #                         # random_state=1,
        #                         learning_rate = 0.1,
        #                         loss = "linear" #"exponential" #"linear"
        #                         )
        ...
        y_predicted = ada_model.predict(X)
...
def predictedOutput(obj):
    '''
    Get ECCDF values for the adaboost scores
    
    Filter the variants to only include common variants set by the DBSNP value RS
    Get the ECDF values for each of these variants using their given adaboost scores 
    Remove the variants with an ECDF value bellow 0.05
        Inputs
            obj    : The ADABoost class object 
            y_pred : The predicted adaboost values 
            X      : Feature dataset 
    '''
    ...
    # sort the variants by their adaboost score 
    RS_subset = RS_subset.sort_values(by = "xgb_score")

    logger.info("Generating ECDF values")

    # Generate the ECDF values using the adaboost scores 
    ...
    cmd = "Threshold score to remove variants: "+ str(max_score)
    logger.info(cmd)
    ...
    # Remove the last column "in_truthset"
    df_bm = df_bm.iloc[:, :-1]
    ...
    ## Save vcf
    #---------------
    out_file = os.path.join(obj.output_path, 'adaboost.vcf')

    # Create a new file ccontaining the VCF header
    header = []
    ...
    # Append the variants to the file containing the VCF header 
...
def main():
    ...
    parser.add_argument("--features", 
                        ...
                        default = "QD,ReadPosRankSum,FS,VPR,VAF,SPLICEADJ,RPT,Homopolymer,Entropy,RNAEDIT")
    
    # Parse the given arguments 
    args = parser.parse_args()
    
    logger.info("Initializing Adaboost class object!")
    ada_object = ADABoost(args)

    ada_object = ada_object.configure_vcf()
    
    ada_object = ada_object.preprocessingData()

    ada_object = ada_object.processTruthSet()
    ...
    # split the dataset
    #---------------------------------------
    X, y, X_train, X_test, y_train, y_test = splitData(ada_object)

    print(X_train.shape, X_test.shape, y_train.shape, y_test.shape)
    print(y_train.value_counts(),'\n', y_test.value_counts())

    ada_object = ada_object.abaBoost(X, y, X_train, X_test, y_train, y_test)


    predictedOutput( obj = ada_object )
